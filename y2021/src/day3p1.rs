...
where T: Clone {
    ...
    (0..len).map(|i| v.iter().map(|r| r[i].clone()).collect()).collect()
}
...
impl BitArray for Vec<Bit> {
    fn most_common_bit(&self) -> Bit {
        if self.iter().map(|x| x.to_weight()).sum::<i32>() >= 0 { Bit::I } else { Bit:: O }
    }

    fn collapse(&self) -> i32 {
        self.iter().rev().zip(0 ..).map(|(b, k)| if let Bit::I = b { 2i32.pow(k) } else { 0 }).sum()
    }
...
fn main() {
    ...
    let matrix: Vec<Vec<Bit>> = io::BufReader::new(file).lines().map(|line| line.unwrap().chars().map(Bit::parse).collect()).collect();
    let tr_matrix = vec_transpose(matrix);
    let gamma = tr_matrix.iter().map(|x| x.most_common_bit()).collect::<Vec<_>>().collapse();
    let epsilon = tr_matrix.iter().map(|x| x.flipped().most_common_bit()).collect::<Vec<_>>().collapse();
