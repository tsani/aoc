...
newtype SizedVector (n :: Nat) a = SizedVector { unSizedVector :: V.Vector a }
  deriving (Functor, Foldable, Traversable, Show, Applicative)
...
-- | Fins have a funny semigroup instance to enable GridZ to be an instance of
-- ComonadApply. This instance just ignores the second input, returning the
-- first one. Such a semigroup instance is incompatible with any law-abiding
-- instance of Monoid.
...
-- | Seeks the store comonad to the first position in the structure where the
-- given comonadic predicate is satisfied.
seekWhere :: (ComonadStore s w, Foldable w) => (w a -> Bool) -> w a -> Maybe (w a)
seekWhere p g = do
  destination <- getFirst . foldMap First $
    g =>> \w -> if p w then Just (pos w) else Nothing
  pure $ seek destination g
